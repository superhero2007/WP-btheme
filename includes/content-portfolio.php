	...
	function mfn_content_portfolio( $query = false, $style = false ){
		...
		if ( $query->have_posts() ){
			while ( $query->have_posts() ){
				...
				$output .= '<li class="image-special-border portfolio-item isotope-item '. $item_class .'">';
					...
					if( $style == 'exposure' ){
						...
						$output .= '<div class="portfolio-item-fw-bg" '. $item_bg .'>';
							...
							if( $style == 'list' ) $output .= '<div class="section_wrapper">'; // style: List | Section Wrapper
								...
								// style: All | Photo ---------------------------------------------
								$output .= '<div class="image_frame scale-with-grid image-special-setting-heights">';
									...
									$output .= '<div class="desc">';
										if( get_post_format() != 'quote') $output .= '<h4><a href="'. get_permalink() .'" class="image_special_title" style="color:black !important;">'. get_the_title() .'</a></h4>';
										$output .= '<p class="image_special_text" style="color:black !important;">'. substr($description, 0, strpos($description,' ',60))." ..." .'</p>';
										$output .= '<a class="button_label image_readmore_button" style="width: 100px !important;  padding: 15px !important;  height: 30px !important;  margin: 0 auto !important;"> Read More </a>';
										$output .= '<a href="'. get_permalink() .'" class="button button_left button_js" style="display:none !important;"><span class="button_icon"><i class="icon-layout"></i></span><span class="button_label" style=" margin: 10px !important;">'. $translate['readmore'] .'</span></a>';
									$output .= '</div>';
								$output .= '</div>';
